...
export default function SolarSystemGraph({
  ...
  const colors = {
    ...
    crossHalo: "#a855f7", // Purple
    edgeCalls: "#38bdf8",
    edgeFunds: "#10b981",
    edgeDefault: "rgba(148, 163, 184, 0.3)"
  };
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      // 1. Draw Subtle Cosmic Starfield
      stars.forEach((star) => {
        ctx.beginPath();
        ctx.arc(star.x, star.y, star.r, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(148, 163, 184, ${star.alpha * 0.4})`;
        ctx.fill();

        // 4-point twinkle cross
        if (star.r > 1.2) {
          ctx.strokeStyle = `rgba(148, 163, 184, ${star.alpha * 0.3})`;
          ctx.lineWidth = 0.5;
          ctx.beginPath();
      ...
      orbitRadii.forEach((r, idx) => {
        ...
        // Orbit Labels cleanly placed on top rim - simple & clear
      ...
      nodesRef.current.forEach((node) => {
        ...
        if (node.id === neutralizedNodeIdRef.current) {
          ...
          ctx.textAlign = "center";
          ctx.strokeText("ARRESTED", node.x, node.y - radius - 8);
          ctx.fillText("ARRESTED", node.x, node.y - radius - 8);
          ctx.restore();
        }
        ...
        // Dark outline shadow for legibility on dark background
        ctx.strokeStyle = "#020617";
        ctx.lineWidth = 3.5;
        ctx.strokeText(node.name, node.x, node.y + radius + 15);

        ctx.fillStyle = isSun ? "#fca5a5" : "#f1f5f9";
        ctx.fillText(node.name, node.x, node.y + radius + 15);

        // Subtitle under Kingpin
        if (isSun) {
          ctx.font = "bold 9px 'Inter', sans-serif";
          ctx.fillStyle = "#ef4444";
          ctx.strokeText("PRIMARY TARGET", node.x, node.y + radius + 27);
          ctx.fillText("PRIMARY TARGET", node.x, node.y + radius + 27);
        }
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden bg-slate-950">
      ...
      {/* Legend Card */}
      <div className="hidden sm:flex absolute top-4 right-4 bg-slate-900/90 backdrop-blur-md px-3.5 py-2 rounded-xl border border-slate-800 shadow-xl text-xs text-slate-300 items-center gap-4 z-10">
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-red-500"></span>
          <span className="font-semibold text-white">Kingpin</span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-full border border-purple-400 border-dashed"></span>
          <span className="text-purple-300">Multi-State Link</span>
        </div>
      </div>
