"""
NetSentry Graph Database Service
Manages Neo4j Cypher operations and maintains resilient in-memory graph analytics (NetworkX).
Supports Neo4j AuraDB (Cloud) and Neo4j 5.x Community (Docker/Local).
"""
...
class Neo4jGraphService:
    def __init__(self):
        ...
        # Resilient in-memory Graph structure
        self.nx_graph = nx.Graph()
        self.nodes_data: Dict[str, Dict[str, Any]] = {}
        self.pending_resolutions: List[Dict[str, Any]] = []
        self.audit_log: List[Dict[str, Any]] = []
        
        # Initialize graph from synthetic data
        self.load_synthetic_dataset()

    def _init_driver(self):
        """Attempts connection to Neo4j instance if configured."""
        try:
            ...
            self.is_connected = False
            print(f" Neo4j connection not available ({e}). Using NetSentry Resilient In-Memory Graph Engine.")
    ...
    def load_synthetic_dataset(self):
        """Loads and indexes the synthetic MH, KA, and FIU data into the graph."""
        self.nx_graph.clear()
        ...
        # 3. Ingest FIU Transactions and create edges
        ...
        # 4. Link associates sharing phones or vehicles
        self._link_shared_identifiers()
        
        # 5. Build HITL Review Queue
    ...
    def _build_resolution_candidates(self):
        ...
        # Candidate 2: Iqbal Painter vs Mohd. Iqbal (HITL candidate ~74%)
        ...
        # Candidate 3: Devanagari transliteration candidate
        aslam_dev = self.nodes_data.get("person_असलम_भाई")
        if aslam_mh and aslam_dev:
            res = resolve_entity_pair(aslam_mh, aslam_dev)
            justification = generate_entity_resolution_justification(
                "Mohd. Aslam", "अस्लम भाई", res, "MH Police", "Devanagari FIR"
            )
            self.pending_resolutions.append({
                "candidate_id": "RES-2026-003",
                "primary_id": "person_mohd_aslam",
                "primary_name": "Mohd. Aslam",
                "primary_dept": "Maharashtra Police (English FIR)",
                "secondary_id": "person_असलम_भाई",
                "secondary_name": "अस्लम भाई",
                "secondary_dept": "Thane Rural (Devanagari FIR)",
                "confidence_score": res["total_score"],
                "adjudication_tier": res["tier"],
                "breakdown": {
                    "levenshtein_similarity": res["breakdown"]["lev"],
    ...
    def get_solar_system_graph(self) -> Dict[str, Any]:
        """
        Computes Graph Centrality and formats nodes into the Solar System hierarchy:
        - Sun (Kingpin): Central node with highest betweenness
        - Orbits: 1 (Lieutenants), 2 (Operatives), 3 (Outer noise)
        """
        ...
        # Find the Kingpin (Sun)
        sorted_nodes = sorted(betweenness.items(), key=lambda x: x[1], reverse=True)
        kingpin_id = sorted_nodes[0][0] if sorted_nodes else "person_mohd_aslam"

        nodes = []
        cross_state_count = 0

        # Filter the most relevant 40-60 nodes for optimal 60fps canvas performance
        top_node_ids = set([n[0] for n in sorted_nodes[:45]])
        # Always include our key story nodes
        top_node_ids.update([
        ...
        # Format Edges between included nodes
        edges = []
