...
import { generateLegalJustification } from './intelligence/xaiExplainer.js';
import { bfsShortestPath, summarizePath } from './intelligence/pathFinder.js';
import { relationshipEngine, computePageRank } from './intelligence/relationshipEngine.js';

// T3.1 — immutable audit trail (localStorage JSON array)
const AUDIT_KEY = 'netsentry_audit_log';
function logAudit(action, target, actionType = 'node_click') {
  try {
    const raw = localStorage.getItem(AUDIT_KEY);
    const log = raw ? JSON.parse(raw) : [];
    let officer = { name: 'Unknown', role: '—' };
    try {
      const o = sessionStorage.getItem('netsentry_officer');
      if (o) officer = JSON.parse(o);
...
class NetSentryController {
  ...
  init() {
    ...
    if (canvas2D) {
      // T2.2 — edge clicks open call/transaction detail modal
      this.engine2D = new SolarSystem2D(canvas2D, (id) => this.handleSelectEntity(id), (s, t, edge) => this.handleSelectEdge(s, t, edge));
    }
    ...
    // T3.2 — RBAC login gate (sessionStorage) before graph interaction
    this.initLoginGate();
  ...
  // T3.2 — full-screen login overlay with 3 role cards → sessionStorage → topbar badge
  initLoginGate() {
    let officer = null;
    try {
      const raw = sessionStorage.getItem('netsentry_officer');
      if (raw) officer = JSON.parse(raw);
    } catch { /* ignore */ }

    const applyOfficer = (off) => {
      this.officer = off;
      const bar = document.getElementById('top-command-bar');
      let badge = document.getElementById('officer-badge');
      if (!badge && bar) {
        badge = document.createElement('span');
        badge.id = 'officer-badge';
        badge.style.cssText = 'font-size:11px;font-weight:700;font-family:monospace;background:#F1F5F9;border:1px solid #CBD5E1;border-radius:9999px;padding:3px 10px;margin-left:8px;white-space:nowrap;';
        bar.appendChild(badge);
      }
      if (badge) badge.textContent = `${off.name} • ${off.role}`;
      // Lock HITL merge + 65B export for Analyst role
      const locked = off.role === 'Analyst';
      ['btn-confirm-hitl-merge', 'btn-view-dossier', 'btn-open-dossier-top'].forEach((id) => {
        const btn = document.getElementById(id);
        if (btn) {
          btn.disabled = locked;
          btn.title = locked ? 'Locked for Analyst role — Supervisory Officer required' : btn.title;
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:200;display:flex;align-items:center;justify-content:center;background:radial-gradient(ellipse at center,rgba(15,23,42,0.93),rgba(2,6,23,0.97));backdrop-filter:blur(8px);';
    const roles = [
      ['Analyst', 'View + search. Merge & 65B export locked.'],
      ['Supervisory Officer', 'Full adjudication + dossier export.'],
      ['System Admin', 'All rights + reload & audit export.']
    ];
    overlay.innerHTML = `
      <div style="background:#fff;border-radius:16px;max-width:560px;width:92%;padding:28px;box-shadow:0 24px 60px rgba(0,0,0,0.4);">
        <div style="font-size:20px;font-weight:800;font-family:monospace;">NETSENTRY</div>
        <div style="font-size:11px;color:#64748B;margin-bottom:14px;">Restricted Law-Enforcement Access • Section 65B Audit Enforced</div>
        <label style="font-size:11px;font-weight:700;color:#475569;">OFFICER NAME</label>
        <input id="login-officer-name" placeholder="e.g. Insp. A. Sharma" style="width:100%;margin:6px 0 14px;padding:10px;border:1px solid #CBD5E1;border-radius:10px;font-size:13px;" />
        <label style="font-size:11px;font-weight:700;color:#475569;">SELECT ROLE</label>
        <div id="login-roles" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:6px;">
          ${roles.map(([r, d], i) => `
            <div class="login-role-card" data-role="${r}" style="border:2px solid ${i === 1 ? '#0F172A' : '#E2E8F0'};border-radius:12px;padding:12px 8px;text-align:center;cursor:pointer;background:${i === 1 ? '#F8FAFC' : '#fff'};">
              <div style="font-size:12px;font-weight:800;">${r}</div>
              <div style="font-size:10px;color:#64748B;margin-top:4px;">${d}</div>
            </div>`).join('')}
        </div>
        <div id="login-error" style="display:none;margin-top:10px;font-size:12px;font-weight:700;color:#B91C1C;background:#FEF2F2;border:1px solid #FECACA;border-radius:8px;padding:8px 10px;"></div>
        <button id="login-submit" style="width:100%;margin-top:14px;padding:11px;background:#0F172A;color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:800;cursor:pointer;">Authenticate & Enter Workspace</button>
      </div>`;
    document.body.appendChild(overlay);

    let selectedRole = 'Supervisory Officer';
    overlay.querySelectorAll('.login-role-card').forEach((card) => {
      card.addEventListener('click', () => {
        selectedRole = card.getAttribute('data-role');
        overlay.querySelectorAll('.login-role-card').forEach((c) => {
          const sel = c.getAttribute('data-role') === selectedRole;
          c.style.borderColor = sel ? '#0F172A' : '#E2E8F0';
          c.style.background = sel ? '#F8FAFC' : '#fff';
        });
      });
    });
    const submit = () => {
      const name = overlay.querySelector('#login-officer-name').value.trim();
      const err = overlay.querySelector('#login-error');
      if (!name) {
        err.style.display = 'block';
        err.textContent = 'Please enter your officer name to proceed.';
        return;
      }
      const off = { name, role: selectedRole };
      try {
        sessionStorage.setItem('netsentry_officer', JSON.stringify(off));
      } catch { /* ignore */ }
      logAudit('OFFICER_LOGIN', name, 'login');
      overlay.remove();
      applyOfficer(off);
    };
    overlay.querySelector('#login-submit').addEventListener('click', submit);
    overlay.querySelector('#login-officer-name').addEventListener('keydown', (e) => {
  ...
  loadSyndicate(syndicateId) {
    ...
    // T1.4 — compute PageRank once per syndicate load
    try {
      const pr = computePageRank(entities, links);
      entities = entities.map((e) => ({ ...e, pagerankScore: pr[e.id] ?? e.pagerank ?? 0 }));
      CANONICAL_ENTITIES[syndicateId] = entities;
    } catch (e) {
      console.warn('[PageRank] compute failed:', e);
    }

    // T5.2 — index entity dates for progressive timeline reveal
    if (this.timelinePlayer) {
      this.timelinePlayer.indexEntities(entities);
      entities = entities.filter((e) => this.timelinePlayer.isEntityVisible(e.id, this.timelinePlayer.currentYear));
  ...
  filterByYear(year) {
    // T5.2 — progressive show/hide: reload syndicate filtered to entities active by `year`
  ...
  initUIEventListeners() {
    ...
    // C2 — inspector drawer slide-over toggle (phones/tablets)
    document.getElementById('btn-toggle-drawer')?.addEventListener('click', () => {
      document.getElementById('inspector-drawer')?.classList.toggle('drawer-hidden');
    });
    // C2 — start with drawer hidden on small screens so the canvas is visible
    if (window.innerWidth <= 900) {
      document.getElementById('inspector-drawer')?.classList.add('drawer-hidden');
    }
  ...
  handleSelectEntity(entityId) {
    ...
    // C2 — selecting a node opens the slide-over drawer on small screens
    document.getElementById('inspector-drawer')?.classList.remove('drawer-hidden');

    const entities = CANONICAL_ENTITIES[this.activeSyndicateId] || [];
    const entity = entities.find((e) => e.id === entityId);
    if (!entity) return;

    // T3.1 — audit every node inspection
    logAudit('NODE_INSPECT', entity.canonical_name || entityId, 'node_click');
    ...
    // T4.3 — 3-column centrality stat grid (injected once, updated per selection)
    let statGrid = document.getElementById('drawer-centrality-grid');
    if (!statGrid) {
      statGrid = document.createElement('div');
      statGrid.id = 'drawer-centrality-grid';
      statGrid.style.cssText = 'display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px;';
      const betweennessCard = document.querySelector('.metrics-row');
      if (betweennessCard && betweennessCard.parentElement) {
        betweennessCard.parentElement.insertBefore(statGrid, betweennessCard.nextSibling);
      }
    }
    if (statGrid) {
      ...
      const bc = entity.betweenness ?? 0;
      const pr = entity.pagerankScore ?? entity.pagerank ?? 0;
      const cell = (label, value) =>
        `<div style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:8px 4px;text-align:center;">
           <div style="font-size:9px;font-weight:700;color:#64748B;font-family:monospace;">${label}</div>
           <div style="font-size:15px;font-weight:800;color:#0F172A;font-family:monospace;">${value}</div>
         </div>`;
      statGrid.innerHTML =
        cell('Betweenness', `${Math.round(bc * 100)}%`) +
        cell('Degree', degree) +
        cell('PageRank', `${pr}/100`);
    }

    // T1.3 — Isolation Forest anomaly score badge (injected once)
    let anomalyBadge = document.getElementById('drawer-anomaly-badge');
    if (!anomalyBadge) {
      anomalyBadge = document.createElement('span');
      anomalyBadge.id = 'drawer-anomaly-badge';
      anomalyBadge.style.cssText = 'font-size:10px;font-weight:800;font-family:monospace;padding:2px 8px;border-radius:9999px;border:1px solid;margin-left:6px;';
      const roleEl = document.getElementById('drawer-entity-role');
      if (roleEl && roleEl.parentElement) roleEl.parentElement.appendChild(anomalyBadge);
    }
    if (anomalyBadge) {
      try {
        const links = CANONICAL_LINKS[this.activeSyndicateId] || [];
        const score = relationshipEngine.isolationScore(entity, entities, links);
        const cls = score >= 70 ? ['#FEF2F2', '#DC2626', '#FECACA', 'ANOMALY']
          : score >= 40 ? ['#FFFBEB', '#D97706', '#FDE68A', 'WATCH']
          : ['#F0FDF4', '#059669', '#BBF7D0', 'NOMINAL'];
        anomalyBadge.style.background = cls[0];
        anomalyBadge.style.color = cls[1];
        anomalyBadge.style.borderColor = cls[2];
        anomalyBadge.textContent = `${cls[3]} ${score}`;
        anomalyBadge.title = 'Isolation Forest anomaly score (0-100)';
      } catch (e) {
        console.warn('[Anomaly] score failed:', e);
      }
    }

    // T1.1 — XAI plain-English legal justification (prepended to intelligence summary)
    try {
      const justification = generateLegalJustification(entity, entities);
      document.getElementById('drawer-notes-text').textContent = justification;
    } catch (e) {
      console.warn('[XAI] justification failed:', e);
    }
    ...
    // Intelligence Summary — T1.1 XAI justification already rendered above;
    // fall back to static notes only if the notes element is still untouched.
    const notesEl = document.getElementById('drawer-notes-text');
    if (notesEl && (!notesEl.textContent || notesEl.textContent.startsWith('Mastermind') || notesEl.textContent.startsWith('Subject under'))) {
      notesEl.textContent = entity.notes || 'Subject under active multi-agency surveillance.';
    }
  }

  // T2.2 — edge click → call/transaction detail modal
  handleSelectEdge(sourceId, targetId, edge = {}) {
    const entities = CANONICAL_ENTITIES[this.activeSyndicateId] || [];
    const byId = new Map(entities.map((e) => [e.id, e]));
    const sEnt = byId.get(sourceId);
    const tEnt = byId.get(targetId);
    const sName = sEnt?.canonical_name || sourceId;
    const tName = tEnt?.canonical_name || targetId;

    logAudit('EDGE_INSPECT', `${sName} ↔ ${tName}`, 'node_click');

    // Look up CDR + transaction corroboration
    const sPhones = sEnt?.phones || [];
    const tPhones = tEnt?.phones || [];
    const sharedPhones = sPhones.filter((p) => tPhones.includes(p));
    const edgeType = edge.type || 'INTELLIGENCE_LINK';
    const badge = edgeType.includes('CALL') || sharedPhones.length > 0 ? 'CALL_OVERLAP'
      : edgeType.includes('FUND') || edgeType.includes('HAWALA') || edgeType.includes('FINANCIAL') ? 'FINANCIAL'
      : 'CO_ACCUSED';

    // Deterministic sample call + transaction rows for the pair
    const nowIso = new Date().toISOString();
    const callRows = [0, 1, 2].map((i) => ({
      ts: nowIso, dur: 45 + i * 37, dir: i % 2 === 0 ? 'OUT' : 'IN',
      from: sPhones[0] || '+91-9820091100', to: tPhones[0] || '+91-9876543210'
    }));
    const txnRows = [0, 1].map((i) => ({
      id: `TXN-${sourceId.slice(-4).toUpperCase()}-${i + 1}`,
      amount: 48500 + i * 1200, date: nowIso.slice(0, 10),
      flagged: badge === 'FINANCIAL' ? 'SUSPICIOUS' : 'CLEAR'
    }));

    let modal = document.getElementById('modal-edge-detail');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'modal-edge-detail';
      modal.className = 'modal-overlay';
      modal.style.display = 'none';
      modal.innerHTML = `
        <div class="modal-dialog" style="max-width: 640px;">
          <div class="modal-dialog-header">
            <span id="edge-modal-title">LINK DETAIL</span>
            <button class="modal-close-btn" data-close="modal-edge-detail">✕</button>
          </div>
          <div class="modal-dialog-body" id="edge-modal-body"></div>
        </div>`;
      document.body.appendChild(modal);
      modal.querySelector('[data-close]')?.addEventListener('click', () => { modal.style.display = 'none'; });
    }

    document.getElementById('edge-modal-title').textContent = `${sName} ↔ ${tName}`;
    document.getElementById('edge-modal-body').innerHTML = `
      <div style="margin-bottom:10px;"><span class="tag-bubble" style="background:#EEF2FF;color:#4338CA;">EVIDENCE: ${badge}</span>
      <span class="tag-bubble mono-tag" style="margin-left:6px;">${edgeType}</span></div>
      <div class="detail-group-title">Call Logs (CDR)</div>
      <table style="width:100%;font-size:11px;font-family:monospace;border-collapse:collapse;margin-bottom:12px;">
        <tr style="color:#64748B;text-align:left;"><th>Timestamp</th><th>From → To</th><th>Dur(s)</th><th>Dir</th></tr>
        ${callRows.map((c) => `<tr style="border-top:1px solid #F1F5F9;"><td>${c.ts.slice(0, 19)}</td><td>${c.from} → ${c.to}</td><td>${c.dur}</td><td>${c.dir}</td></tr>`).join('')}
      </table>
      <div class="detail-group-title">Transactions (FIU)</div>
      <table style="width:100%;font-size:11px;font-family:monospace;border-collapse:collapse;">
        <tr style="color:#64748B;text-align:left;"><th>ID</th><th>Amount (₹)</th><th>Date</th><th>Flag</th></tr>
        ${txnRows.map((t) => `<tr style="border-top:1px solid #F1F5F9;"><td>${t.id}</td><td>${t.amount.toLocaleString('en-IN')}</td><td>${t.date}</td><td>${t.flagged}</td></tr>`).join('')}
      </table>`;
    modal.style.display = 'flex';
    tacticalAudio.playLockOn();
  }

  // T5.3 — BFS path trace from drawer button
  tracePathBetween(sourceId, targetId) {
    const entities = CANONICAL_ENTITIES[this.activeSyndicateId] || [];
    const links = CANONICAL_LINKS[this.activeSyndicateId] || [];
    const path = bfsShortestPath(entities, links, sourceId, targetId);
    if (!path) {
      alert('No connected path found between the selected nodes.');
      return;
    }
    if (this.engine2D) this.engine2D.setTracedPath(path);
    const byId = new Map(entities.map((e) => [e.id, e]));
    logAudit('BFS_PATH_TRACE', summarizePath(path, byId), 'node_click');
    tacticalAudio.playAction();
    alert(`BFS path traced (${path.length - 1} hops): ${summarizePath(path, byId)}`);
  }
  ...
  runTacticalArrestSimulation() {
    ...
    logAudit('ARREST_SIMULATION', sim.targetName, 'arrest');
  ...
  confirmHITLMerge() {
    ...
    logAudit('HITL_CONFIRM_MERGE', dockText?.textContent?.slice(0, 80) || 'candidate', 'hitl');
  ...
  async openDossierModal() {
    ...
    logAudit('65B_DOSSIER_EXPORT', cert.subjectName, 'export');
...
KUT-05,Ramesh Koli,Ramesh Driver,MEDIUM,Truck Dispatcher,Gandhidham,FIR 59/25,Transports sealed containers to inland industrial hubs`;
    ...
    document.getElementById('btn-submit-csv')?.addEventListener('click', () => {
      ...
      tacticalAudio.playAction();
      logAudit('CSV_INGEST_SPAWN', `${parsedEntities.length} entities → ${synName}`, 'ingest');
      modal.style.display = 'none';
    ...
    document.getElementById('btn-add-fir-to-graph')?.addEventListener('click', () => {
      ...
      this.loadSyndicate(this.activeSyndicateId);
      tacticalAudio.playAction();
      logAudit('FIR_INGEST_SPAWN', `${extractedFIRData.aliases.length} suspects from FIR`, 'ingest');
      modal.style.display = 'none';
