...
export class RelationshipEngine {
  ...
  /**
   * T1.3 — Isolation Forest-style anomaly score (0–100) for a suspect.
   * Feature vector: [betweenness, degree, phoneCount, jurisdictionCount, txnVelocity, burstDelta]
   * Uses a lightweight isolation approximation: normalized distance of the
   * entity's feature vector from the corpus median, scaled to 0–100.
   */
  isolationScore(entity, corpus = [], links = []) {
    const degreeOf = (id) =>
      (links || []).filter((l) => {
    ...
    const featuresOf = (e) => [
      ...
      degreeOf(e.id),
      (e.phones || []).length,
      e.jurisdictionCount ?? e.jurisdiction_count ?? ((e.agencies || []).length || (e.is_cross_jurisdiction ? 2 : 1)),
      e.txn_velocity ?? e.txnVelocity ?? (e.firs || []).length,
      e.burst_delta ?? e.burstDelta ?? 0
    ];
    ...
    // Column-wise median + MAD-style scale
    const medians = [];
    const scales = [];
    ...
    // Map distance → 0–100 (sigmoid centered at dist≈1.5)
...
/**
 * T1.4 — PageRank centrality computation (power iteration).
 ...
 * @param {number} iterations - default 50
 * @returns {object} map of nodeId → normalized 0–100 PageRank score
 */
export function computePageRank(nodes, edges, dampingFactor = 0.85, iterations = 50) {
  const ids = (nodes || []).map((n) => n.id);
  const n = ids.length;
  const out = {};
  if (n === 0) return out;

  const outLinks = new Map(ids.map((id) => [id, []]));
  (edges || []).forEach((e) => {
    const s = typeof e.source === 'object' ? e.source.id : e.source;
    const t = typeof e.target === 'object' ? e.target.id : e.target;
    if (outLinks.has(s) && outLinks.has(t)) {
      outLinks.get(s).push(t);
    }
  });
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    let dangling = 0;
    ids.forEach((id) => {
      if (outLinks.get(id).length === 0) dangling += rank[id];
    });
    ids.forEach((id) => {
      next[id] = (1 - dampingFactor) / n + (dampingFactor * dangling) / n;
    });
    ids.forEach((id) => {
      const outs = outLinks.get(id);
      if (outs.length === 0) return;
      const share = (dampingFactor * rank[id]) / outs.length;
      outs.forEach((t) => { next[t] += share; });
    });
    rank = next;
  }
  ...
  ids.forEach((id) => {
    out[id] = Math.round((rank[id] / max) * 100);
  });
  return out;
}
