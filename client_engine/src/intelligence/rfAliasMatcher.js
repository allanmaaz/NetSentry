...
/**
 * T1.2 — Backend-backed inference: calls POST /api/ml/match (trained sklearn
 * Random Forest) with localStorage-cached fallback to client-side weights
 * when the backend is unreachable (air-gapped mode).
 *
 * @param {object} entityA - {name, phones[], vehicles[]}
 * @param {object} entityB - {name, phones[], vehicles[]}
 * @param {string} apiBase - backend base URL (default localhost:8000)
 * @returns {Promise<object>} {confidenceScore, decision, featureBreakdown, source}
 */
const ML_CACHE_KEY = 'netsentry_ml_match_cache';

function readMatchCache(key) {
  try {
    const cache = JSON.parse(localStorage.getItem(ML_CACHE_KEY) || '{}');
...
function writeMatchCache(key, value) {
  try {
    ...
    if (keys.length > 200) delete cache[keys[0]]; // bounded cache
...
export async function predictRecordLinkageRemote(entityA, entityB, apiBase = 'http://127.0.0.1:8000') {
  const nameA = entityA.name || entityA.canonical_name || '';
  const nameB = entityB.name || entityB.canonical_name || '';
  const cacheKey = `${nameA}__${nameB}`;

  try {
    const res = await fetch(`${apiBase}/api/ml/match`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name_a: nameA,
        name_b: nameB,
        phones_a: entityA.phones || [],
        phones_b: entityB.phones || [],
        vehicles_a: entityA.vehicles || [],
        vehicles_b: entityB.vehicles || []
      })
    });
    ...
    const out = {
      ...
      featureBreakdown: (data.feature_breakdown || []).map((f) => ({
        name: f.name,
        weight: f.weight,
        value: f.value,
        score: f.value
      })),
      legalExplanation: `Backend Random Forest verdict: ${Math.round(data.confidence_score * 100)}% (${data.decision}). Model: ${data.model}.`,
      source: 'backend'
    };
    writeMatchCache(cacheKey, out);
    return out;
  } catch (err) {
    // localStorage fallback → cached verdict or local weighted inference
    const cached = readMatchCache(cacheKey);
    if (cached) return { ...cached, source: 'cache' };
    const local = predictRecordLinkage(entityA, entityB);
